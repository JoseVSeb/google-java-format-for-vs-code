import type { ConfigurationChangeEvent, ExtensionContext } from "vscode";
import { workspace } from "vscode";

const SECTION = `java.format.settings.google`;

export type GoogleJavaFormatVersion = `${number}.${number}.${number}` | "latest";

export type GoogleJavaFormatMode = "jar-file" | "native-binary"; // TODO: | "background-service" — future scope

export type GoogleJavaFormatStyle = "google" | "palantir";

export interface GoogleJavaFormatConfiguration {
  executable?: string;
  version?: GoogleJavaFormatVersion;
  mode?: GoogleJavaFormatMode;
  style?: GoogleJavaFormatStyle;
  extra?: string;
}

export class ExtensionConfiguration implements GoogleJavaFormatConfiguration {
  executable?: string;
  version?: GoogleJavaFormatVersion;
  mode?: GoogleJavaFormatMode;
  style?: GoogleJavaFormatStyle;
  extra?: string;
  readonly subscriptions: ((config: GoogleJavaFormatConfiguration) => void)[] = [];

  constructor(private context: ExtensionContext) {
    this.load = this.load.bind(this);
    this.configurationChangeListener = this.configurationChangeListener.bind(this);
    this.load();
  }

  subscribe() {
    this.context.subscriptions.push(
      workspace.onDidChangeConfiguration(this.configurationChangeListener),
    );
  }

  private load() {
    const { subscriptions: _subs, ...config } = workspace.getConfiguration(SECTION);
    Object.assign(this, config);
  }

  private async configurationChangeListener(event: ConfigurationChangeEvent) {
    if (!event.affectsConfiguration(SECTION)) {
      return;
    }

    this.load();
    for (const fn of this.subscriptions) {
      fn(this);
    }
  }
}
